...
class ImgController extends BaseController {
	...
	public function admin() {
		...
		$images = PostImage::get_images(Constant::$ADMIN_PAGESIZE);

		// if (count($images) <= 0) {
		// 	$images = null;
		// } else {
		// 	$images = Post::add_meta($posts);
		// }

		$view = View::make('other/img_admin',
			array('title' => '图片管理',
				'username' => $username,
				'images' => $images,
				'menu' => 'img',
	...
	public function delete($iid) {
		$err = '';
		try {
			Log::info('delete image ' . $iid);
			PostImage::delete_imgandfile($iid);
		} catch (Exception $e) {
			$err = $e->getMessage();
		}
		if (strlen($err) > 0) {
			$resp = Redirect::action('ErrorController@show', array($err));
			Log::error($err);
		} else {
			$resp = Redirect::route('img_admin');
		}
		return $resp;
	}
	...
	/**
	 * 上传图片，转化图片为2:1，回传url,iid
	 * 保存两份，RAW较大保存待用，回传裁剪压缩过的url，用于裁剪，同时生成meta信息
	 * @return unknown
	 */
