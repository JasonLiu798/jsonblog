...
<form method="post" action="{{url()}}/admin/post/update/{{$post->post_id}}" accept-charset="utf-8" role="form" id="update_post_form">
	<input type="hidden" name="method" id="method" value="update" />
	<input type="hidden" name="post_id" id="post_id" value="{{ $post->post_id }}" />
	<input type="hidden" name="post_tag_ids" id="post_tag_ids" value="{{ is_null($post->post_tag_id)?'':$post->post_tag_id }}" />
		...
		<select class="form-control" name="category" id="category">
			@if(!is_null($category) && count($category)>0)
				@foreach($category as $cat)
					<option id="category{{$cat->term_id}}" value="{{$cat->term_id }}" @if($cat->selected==1)  selected @endif>{{ $cat->name }}</option>
				@endforeach
			@else
	...
	<div class="form-group">
		...
		<div>
			<img id="cover_img_preview_inpage" src="{{ is_null($post->cover_img_url)?'':$post->cover_img_url }}"/>
		</div>
	</div>
